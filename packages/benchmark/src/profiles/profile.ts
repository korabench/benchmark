...
// Configs are inline (not slugs into a registry) so a profile file is a
...
//
// The model lives here, next to packs, so that every harness (the CLI, the
// kora-infra worker) shares one schema and one hash formula. Loading a profile
// from disk is the CLI's business (`packages/cli/src/profiles/loadProfile.ts`).
//
